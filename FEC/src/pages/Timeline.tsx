  ...
  return (
    <div className="flex h-full flex-col">
      <div className="border-b border-zinc-800 px-6 py-4">
        ...
        <p className="text-sm text-zinc-500">{t("timeline.subtitle")}</p>
      </div>
          ...
          <>
              ...
              <div className="flex items-end gap-1.5 overflow-x-auto pb-1" style={{ minHeight: 140 }}>
                {months.map(([key, rs]) => {
                  ...
                  return (
                    ...
                    >
                      <span className={"text-[10px] font-semibold " + (active ? "text-indigo-300" : "text-zinc-500")}>
                        {rs.length}
                      </span>
                      <div
                        className={
                          ...
                          (active
                            ? "bg-indigo-500"
                            : "bg-indigo-600/40 group-hover:bg-indigo-500/70")
                        }
                        style={{ height: Math.round((rs.length / maxCount) * 96) + 4 }}
                      />
                      <span className={"text-[9px] " + (active ? "text-indigo-300" : "text-zinc-600")}>
                        {fmtMonthShort(key)}
                      </span>
                    </button>
                  );
            ...
            <div className="mt-6">
              <div className="mb-3 flex items-center gap-3">
                ...
                {selected && (
                  <button onClick={() => setSelected(null)} className="text-xs text-indigo-400 hover:text-indigo-300">
                    {t("timeline.showAll")}
                  </button>
                )}
                <span className="text-xs text-zinc-600">{shown.length}</span>
              </div>
